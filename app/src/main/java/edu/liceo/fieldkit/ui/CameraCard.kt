...
fun CameraCard() {
    ...
    LaunchedEffect(shake) {
        ...
        if (isShake(shake) && now - lastShot > 1500 && cameraPermission.status == edu.liceo.fieldkit.permissions.PermStatus.Granted) {
            lastShot = now
        ...
        Column(
            ...
            PermissionGate(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    // Show Torch button for the system, and a Force Torch for emulator testing
                    if (cam?.cameraInfo?.hasFlashUnit() == true) {
                        ...
                        // Temporary button to see the logic works on emulator
                        OutlinedButton(onClick = { torchOn = !torchOn }) {
                            Text(if (torchOn) "F-Torch Off" else "F-Torch On")
                        }
                    }
                }
                
                Text("Shake Capture: ACTIVE", style = MaterialTheme.typography.labelSmall)
